...
function modelMILP(instanceMILP, binary::Bool = true, solver = Gurobi.Optimizer)
    ...
    @constraint(m, c2[r=1:R,j=1:length(B[r])], sum(X[r, j, k] for k in Oj[r][j].debut:Oj[r][j].fin) == 1 )
    @constraint(m, c3[r=1:R,k=1:O], sum(X[r, j, k] for j in U[r][k]) <= 1 )
    @constraint(m, c4[r=1:R,j=1:length(B[r])-1], sum(k * X[r, j, k] for k in Oj[r][j].debut:Oj[r][j].fin) <= sum(k * X[r, j+1, k] for k in Oj[r][j+1].debut:Oj[r][j+1].fin))
